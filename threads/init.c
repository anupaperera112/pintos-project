...
#endif
char key_array[128];
char key_command[][128] = {"whoami", "shutdown", "time", "ram", "thread", "priority", "exit"};
int key_index = 0;
void command_execute(int i){
  switch (i)
  {
  case 0:
    printf("Anupa Perera \n210462E");
  ...
  case 2:
    printf("Number of seconds passed since Unix epoch is %d", timer_ticks());
    break;
  ...
  case 5:
    ...
    printf("current thread priority is %d" , thread_get_priority());
    break;
...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    for(int i = 0; i < sizeof(key_command)/sizeof (key_command[0]); i++){
      int result = strcmp(key_command[i], key_array);
